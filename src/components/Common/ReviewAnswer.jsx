...
import { RenderHtmlContent, decryptAnswer, handleBookmarkClick, imgError, reportQuestion } from "src/utils";
...
const ReviewAnswer = ({ questions, goBack, reportquestions, showLevel, latex, showBookmark }) => {
    ...
    return (
        <>
            ...
            <div className="morphisam row justify-content-center mb-3">
            ...
            <div className="text-center review-answer-data">
                <small className="review-latext-note">
                    {questions[currentQuestion]?.note ?
                        <><span class="text-nowrap">{t("note")} :- &nbsp; </span>{systemconfig.latex_mode === "1" ? <p><RenderHtmlContent htmlContent={questions[currentQuestion]?.note} /></p> : <p dangerouslySetInnerHTML={{ __html: questions[currentQuestion]?.note }}></p>}</>
                        :
                        ""
                    }
                </small>
